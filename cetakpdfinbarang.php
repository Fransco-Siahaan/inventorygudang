...
$query = "SELECT * FROM tambah" . $whereClause . " ORDER BY tanggal DESC";
...
$pdf->Cell(5, 5, ':', 0, 0);
$pdf->Cell(0, 5, 'SAPAT RESELLER - Tanah Jawa', 0, 1);
...
$header = array('No', 'Kode Barang', 'Nama Barang', 'Jumlah', 'Satuan', 'Harga', 'Subtotal');
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell($widths[2], 6, $row['nmbarang'], 'LR', 0, 'L');
    $pdf->Cell($widths[3], 6, $row['jumlah'], 'LR', 0, 'C'); // Jumlah saja
    $pdf->Cell($widths[4], 6, $row['satuan'], 'LR', 0, 'C'); // Satuan saja
...
// Total Pembayaran dengan format yang lebih baik
$terbilang = terbilang($total) . ' Rupiah';
$pdf->SetFont('helvetica', '', 10);
$pdf->Cell(120, 7, 'Terbilang: ' . $terbilang, 0, 0);
$pdf->Ln(7); // Beri jarak sebelum JUMLAH
$pdf->Cell(120, 7, '', 0, 0);
